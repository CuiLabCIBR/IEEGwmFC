...
#main function
Data <- readMat('Fig3e.mat')
DsiSCseegFC = data.frame(DSISC = as.numeric(Data$Fig3e[1, 1:511]))
DsiSCseegFC$SEEGFC = as.numeric(Data$Fig3e[2, 1:511])
p <- ggplot(DsiSCseegFC, aes(DSISC, SEEGFC)) + 
  geom_point(color="gray78", size=2) + 
  labs(x = "log(SC)", y = "SEEG FC") +
  geom_smooth(color = 'black', formula = y ~ x, fill = "gray78", method = "lm", size=1) + 
  scale_x_continuous(breaks=seq(-2, 4, 1), labels = c(-2, -1, 0, 1, 2, 3, 4), limits = c(-1.5,2.5)) +
  scale_y_continuous(breaks=seq(-0.6, 0.9, 0.3), labels = c(-0.6, -0.3, 0, 0.3, 0.6, 0.9), limits = c(-0.7, 0.8)) +
  scale_color_npg() + 
  scale_fill_npg() +
  theme_niwot()
p
#save fig
ggsave("Fig3e_dsisc_seegfc.png", plot = p, dpi=300, width = 6.2, height = 5.3)
